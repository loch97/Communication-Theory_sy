Fs=1e4;                        %采样频率
len=20;                        %码元长度
in=randint(1,len,[-3 3]);      %产生初始码元序列
sig=[];	
out=[];
for t = 1:2000                   % 产生双极性归零码
    n = fix(t/100);
    if n==0
        in_a(t) = 0;
    else
...
title('基带信号-双极性不归零码','FontWeight','bold','FontSize',20);
xlabel('t/s','FontSize',18);
axis([100,2100,-3.5,3.5]);
